...
class LightBumpData(object):
    def __init__(self,data):
        self.__data = data

    def __left(self):
        return TwoBytes(self.__data[0:2]).to_uint16()
    left = property(__left)

    def __front_left(self):
        return TwoBytes(self.__data[2:4]).to_uint16()
    front_left = property(__front_left)

    def __center_left(self):
        return TwoBytes(self.__data[4:6]).to_uint16()
    center_left = property(__center_left)

    def __center_right(self):
        return TwoBytes(self.__data[6:8]).to_uint16()
    center_right = property(__center_right)

    def __front_right(self):
        return TwoBytes(self.__data[8:10]).to_uint16()
    front_right = property(__front_right)
...
class RoombaAPI(object):
    ...
    def drive_direct(self, vel_right, vel_left):
        vel_R = ConvertToBytes(int16(vel_right).value)
        vel_L = ConvertToBytes(int16(vel_left).value)
        self.send_to_roomba([
            145,
            vel_R.high_byte(),
            vel_R.low_byte(),
    ...
    def __lightbumps(self):
        ...
        while(x < end):
            data.append(ord(output[x]))
            x = x + 1
        return LightBumpData(data)

    lightbumps = property(__lightbumps)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print "lightbump:L" + str(x.lightbumps.left)
        print "lightbump:FL" + str(x.lightbumps.front_left)
        print "lightbump:CL" + str(x.lightbumps.center_left)
        print "lightbump:CR" + str(x.lightbumps.cetner_right)
        print "lightbump:FR" + str(x.lightbumps.front_right)
        print "lightbump:R" + str(x.lightbumps.right)
